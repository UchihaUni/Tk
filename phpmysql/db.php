...
class DB{
    ...
    public function getAuthors(){
        ...
        $sql = $connect->query('SELECT last_name, first_name from authors order by last_name');
        $sql->execute();
        return $sql->fetchAll();
    }
    public function createAuthor($request){
        ...
        try{
            ...
            $connect->exec('insert into authors values (null,"'.$request['last_name'].'",
            "'.$request['first_name'].'",
            "'.$request['father_name'].'",
            "'.mb_substr($request['first_name'], 0, 1).'.",now(), now())');
            $connect->commit();
            return json_encode([
                'error'=>false,
